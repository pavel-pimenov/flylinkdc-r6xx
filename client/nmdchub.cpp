...
OnlineUserPtr NmdcHub::getUser(const string& aNick, bool p_hub, bool p_first_load)
{
	...
	if (l_find.second == false) // Íå ïðîøëà âñòàâêà ò.ê. òàêîé íèê óæå åñòü â ìàïå?
	{
...
//			dcassert(m_users.count(aNick) == 0);
	...
	if (!ou->getUser()->getCID().isZero())
	{
		...
		//  is_all_my_info_loaded() áåç true íå íà÷èíàåò êà÷àòü ïðè çàãðóçêå
...
void NmdcHub::clearUsers()
{
	...
	else
	{
		...
		for (auto i = u2.cbegin(); i != u2.cend(); ++i)
		{
			...
			// Âàðèàíòû
			// - ñêàðìëèâàòü þçåðîâ ìàññèâîì
			// - Äåðæàòü þçåðîâ â íåñêîëüêèõ êîíòåéíåðàõ äëÿ êàæäîãî õàáà îòäåëüíî
			// - ïðîðàáîòàòü êîìàíäó íà óáèâàíèå âñåé ìàïû ñðàçó áåç ïîèñêà
		}
	}
}
//==========================================================================================
...
{
	const StringTokenizer<string> tok(tag, ',', 4); // TODO - óáðàòü ðàçáîð òîêåíîâ. ñäåëàòü ïðîñòîå ñêàíèðîâàíèå â öèêëå â ïîèñêå çàïÿòûõ
	string::size_type j;
	id.setLimit(0);
	for (auto i = tok.getTokens().cbegin(); i != tok.getTokens().cend(); ++i)
	{
		...
		else
		{
			...
			// dcassert(0);
			// TODO - ñáðîñ îøèáî÷íûõ òýãîâ â êà÷åñòâå ñòàòû?
		}
...
void NmdcHub::searchParse(const string& param, bool p_is_passive)
{
	...
	if (!p_is_passive)
	{
		...
		if (m != string::npos)
		{
			const auto k = param.find("?9?TTH:", m); // Åñëè èäåò çàïðîñ ïî TTH - ïðîïóñêàåì áåç ïðîâåðêè
			if (k == string::npos)
			{
	...
	if (p_is_passive)
	{
		// seeker â íà÷àëå ìîæåò íå ñîäåðæàòü "Hub:" - ïàäàåì
...
void NmdcHub::chatMessageParse(const string& p_line)
{
	...
	if (!l_user)
	{
		...
		// åñëè þçåð ïîäñòàâíîé - íå ñîçäàåì åãî â ñïèñêå
	}
	...
	if (!chatMessage->m_from)
	{
		if (l_user)
		{
			chatMessage->m_from = l_user; ////getUser(nick, false, false); // Òóò âíóòðè ñíîâà èäåò ïîèñê findUser(nick)
...
{
	const StringTokenizer<string> st(param, ' '); // TODO óáðàòü òîêåíû. ñäåëàòü ïîèñêîì.
	...
	/*
	<Mer> [00:27:44] *** Ñîåäèí¸í
	- [00:27:47] <MegaHub> Âðåìÿ ðàáîòû: 129 äíåé 8 ÷àñîâ 23 ìèíóò 21 ñåêóíä. Ïîëüçîâàòåëåé îíëàéí: 10109
	- [00:27:51] <MegaHub> Operation timeout (ValidateNick)
	- [00:27:52] *** [Hub = dchub://hub.o-go.ru] Ñîåäèíåíèå çàêðûòî
	{
...
void NmdcHub::userIPParse(const string& p_ip_list)
{
	if (!p_ip_list.empty())
	{
			...
			for (auto it = sl.cbegin(); it != sl.cend() && !ClientManager::isBeforeShutdown(); ++it)
			{
				   ...
				   )
				{
					   ...
					   )
					{
						...
						if (
						    // (l_avdb_result & Identity::VT_NICK) && (l_avdb_result & Identity::VT_SHARE) &&  (l_avdb_result & Identity::VT_IP) || - ýòî îñòûëàòü íå íóæíî îíè è òàê â áàçå óæå åñòü
						    l_is_nick_share ||
						    l_is_ip_share ||
						    l_is_nick_ip
						    //! íåëüçÿ ïðîâåðÿòü òîëüêî ïî IP (l_avdb_result & Identity::VT_IP)
						)
							if (!CFlyServerConfig::g_antivirus_db_url.empty())
							{
								// http://te-home.net/avdb.php?do=send&size=<ðàçìåð øàðû>&addr=<èï àäðåñ, íå îáÿçàòåëüíî>&nick=<íèê þçåðà>&path=<ïóòü ê âèðóñàì, íå îáÿçàòåëüíî>
						...
						if (m_isAutobanAntivirusNick)
						{
							...
							13:38:03 Hub:   [Incoming][162.211.230.164:411]     <PtokaX> *** ork5005 ñ IP 95.183.29.221 áûë êèêíóò FlylinkDC-dev.
		...
		// TODO - ñëàòü ñîîáùåíèÿ î ñìåíå òîëüêî IP
		// fire_user_updated(v);
...
void NmdcHub::opListParse(const string& param)
{
	if (!param.empty())
	{
		...
		fire_user_updated(v); // íå óáèðàòü - ÷åðåç ýòó êîìàíäó øëþò "÷àñû"
		updateCounts(false);
...
//			}
	...
	else
	{
		...
		l_is_search = cmd == "Search"; // TODO - ýòîãî áîëüøå íå áóäåò - ïîõåðèòü
	...
	if (l_is_search && ClientManager::isStartup() == false)
	{
		dcassert(0);  // Èñïîëüçóåì void NmdcHub::on(BufferedSocketListener::SearchArrayFile
	...
	else if (cmd == "NickList") // TODO - óáèòü
	{
		nickListParse(param);
	}
	else if (cmd == "OpList")
	...
	{
		dcassert(0); // Îáðàáîòêó ZOn ïåðåíåñ â BufferedSocket ÷òîáû íå çâàòü ëèøíèé Listener
	}
	...
	{
		// Ãäå-òî îøèáêà â ïëàãèíå - ìíîãî ñïàìà èäåò íà ñåðâåð - îòðóáèë íàõðåí
	...
	else if (cmd == "BadNick")
	{
		...
		/*
		$BadNick TooLong 64        -- íèê ñëèøêîì äëèííûé, ìàêñèìàëüíàÿ äîïóñòèìàÿ äëèíà íèêà 64 ñèìâîëà     (ôëàé ñ÷èòàåò ñêîëüêî ó íåãî â íèêå ñèìâîëîâ è óáèðàåò ëèøíèå, òàê ÷òîá îñòàëîñü ìàêñèìóì 64)
		$BadNick TooShort 3        -- íèê ñëèøêîì êîðîòêèé, ìèíèìàëüíàÿ äîïóñòèìàÿ äëèíà íèêà 3 ñèìâîëà     (ôëàé ñ÷èòàåò ñêîëüêî ó íåãî â íèêå ñèìâîëîâ è äîáàâëÿåò íåõâàòàþùèå, òàê ÷òîá áûëî ìèíèìóì 3)
		$BadNick BadPrefix        -- ó íèêà ëèøíèé ïðåôèêñ, õàá õî÷åò íèê áåç ïðåôèêñà      (ôëàé óáåðàåò âñå ïðåôèêñû èç íèêà)
		$BadNick BadPrefix [ISP1] [ISP2]        -- ó íèêà íåõâàòàåò ïðåôèêñà, õàá õî÷åò íèê ñ ïðåôèêñîì [ISP1] èëè [ISP2]      (ôëàé äîáàâëÿåò ñëó÷àéíûé èç ïåðå÷èñëåíûõ ïðåôèêñîâ ê íèêó)
		$BadNick BadChar 32 36        -- íèê ñîäåðæèò çàïðåùåííûå õàáîì ñèìâîëû, õàá õî÷åò íèê â êîòîðîì íå áóäåò ïåðå÷èñëåíûõ ñèìâîëîâ      (ôëàé óáèðàåò èç íèêà âñå ïåðå÷èñëåíûå áàéòû ñèìâîëîâ)
		*/
		dcassert(m_client_sock);
		if (m_client_sock)
			m_client_sock->disconnect(false);
...
bool NmdcHub::resendMyINFO(bool p_always_send, bool p_is_force_passive)
{
	...
	{
		if (m_modeChar == 'P')
			return false; // Óõîäèì èç îáíîâëåíèÿ MyINFO - óæå íàõîäèìñÿ â ïàññèâíîì ðåæèìå
	}
...
void NmdcHub::myInfo(bool p_always_send, bool p_is_force_passive)
{
	...
	if (p_always_send || l_is_change_my_info || l_is_change_fly_info)
	{
		...
		if ((m_supportFlags & SUPPORTS_EXTJSON2) && l_is_change_fly_info)
		{
			...
			Text::trim(l_json_str); // TODO - óáðàòü â êîíöå ïðîáåë â json
	...
	try
	{
		...
		else
		{
			if (ou->getIdentity().setExtJSON(param))
			{
				...
				ou->getIdentity().setExtJSONQueueSrc(l_root["QueueSrc"].asInt64()); //TODO - âðåìåííû áàã - òóò 32 áèòà
				ou->getIdentity().setExtJSONTimesStartCore(l_root["StartCore"].asInt64());  //TODO òóò òîæå 32 áèòà
				ou->getIdentity().setExtJSONTimesStartGUI(l_root["StartGUI"].asInt64()); //TODO òóò òîæå 32 áèòà
				
				if (p_is_disable_fire == false)
				{
					updatedMyINFO(ou); // TODO îáíîâëÿòü òîëüêî JSON
				}
			}
		}
		return true;
...
void NmdcHub::myInfoParse(const string& param)
{
	...
	OnlineUserPtr ou = getUser(l_nick, false, m_bLastMyInfoCommand == DIDNT_GET_YET_FIRST_MYINFO); // Ïðè ïåðâîì êîííåêòå èñêëþ÷àåì ïîèñê
...
#endif
				{
					updateFromTag(ou->getIdentity(), l_tag, l_is_version_change); // òÿæåëàÿ îïåðàöèÿ ñ òîêåíàìè. TODO - îïòèìèçíóòü
	...
	if (j == string::npos)
		return;
#ifdef FLYLINKDC_USE_CHECK_CHANGE_MYINFO
	// Ïðîâåðèì ÷òî ìåíÿåòñ òîëüêî øàðà
	...
	auto l_share_size = Util::toInt64(param.c_str() + i); // Èíîãäà øàðà áûâàåò == -1 http://www.flickr.com/photos/96019675@N02/9732534452/
	if (l_share_size < 0)
	{
		l_share_size = 0;
#ifdef FLYLINKDC_BETA
	...
	{
		extJSONParse(l_ext_json_param, true); // true - íå çîâåì ClientListener::UserUpdatedMyINFO
		{
			CFlyWriteLock(*m_cs);
		...
		for (auto i = p_search_array.begin(); i != p_search_array.end(); ++i)
		{
			if (i->m_toSRCommand)
			{
				...
				if (i->m_is_passive)
				{
					...
					{
						// Ñôîðìèðóåì îòâåò íà ïàññèâíûé çàïðîñ
				...
				{
					// Çàïðîñû ïî TTH - ïîêèäûâàåì ÷åðåç êîðîòêî-æèâóùèé ôèëüòð, ÷òîáû èñêëþ÷èòü ëèøíèé äóáëèêàòíûé
					// ïîèñê è ïàðàçèòíûé UDP òðàôèê â îáðàòíóþ ñòîðîíó
...
void NmdcHub::on(BufferedSocketListener::SearchArrayFile, const CFlySearchArrayFile& p_search_array) noexcept
{
	if (!ClientManager::isBeforeShutdown())
	{
		for (auto i = p_search_array.cbegin(); i != p_search_array.end(); ++i)
		{
			...
			// TODO - íàó÷èòñÿ îáðàáàòûâàòü - ïîèñê ïî TTH ñ îãðàíè÷åíèÿìè ïî ðàçìåðó
			// "x.x.x.x:yyy T?F?57671680?9?TTH:A3VSWSWKCVC4N6EP2GX47OEMGT5ZL52BOS2LAHA"
			if (!ClientManager::isBeforeShutdown())
			{
				searchParse(i->m_raw_search, i->m_is_passive); // TODO - ó íàñ óæå åñòü ðàñïàðñåííîå
