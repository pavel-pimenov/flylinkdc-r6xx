...
CryptoManager::~CryptoManager() {
	...
	for (int i = KEY_FIRST; i != KEY_RSA_2048; ++i) {
		if (g_tmpKeysMap[i]) DH_free((DH*)g_tmpKeysMap[i]);
	}
	
	for (int i = KEY_RSA_2048; i != KEY_LAST; ++i) {
		if (g_tmpKeysMap[i]) RSA_free((RSA*)g_tmpKeysMap[i]);
	}
...
ByteVector CryptoManager::calculateSha1(const string& aData) noexcept {
...
DH* CryptoManager::getTmpDH(int keyLen) {
	...
	switch (keyLen) {
		case 2048: {
			...
			DH_set0_pqg(tmpDH, BN_bin2bn(dh2048_p, sizeof(dh2048_p), 0), NULL, BN_bin2bn(dh_g, sizeof(dh_g), 0));
			break;
		}
		...
		case 4096: {
			...
			DH_set0_pqg(tmpDH, BN_bin2bn(dh4096_p, sizeof(dh4096_p), 0), NULL, BN_bin2bn(dh_g, sizeof(dh_g), 0));
			break;
		}
		default: {
			DH_free(tmpDH);
...
void CryptoManager::loadCertificates() noexcept {
	...
	if (
	    ssl::SSL_CTX_use_certificate_file(serverContext, cert.c_str(), SSL_FILETYPE_PEM) != SSL_SUCCESS ||
	    ssl::SSL_CTX_use_certificate_file(serverALPNContext, cert.c_str(), SSL_FILETYPE_PEM) != SSL_SUCCESS ||
	    ssl::SSL_CTX_use_certificate_file(clientALPNContext, cert.c_str(), SSL_FILETYPE_PEM) != SSL_SUCCESS ||
	    ssl::SSL_CTX_use_certificate_file(clientContext, cert.c_str(), SSL_FILETYPE_PEM) != SSL_SUCCESS
	)
	{
	...
	if (
	    ssl::SSL_CTX_use_PrivateKey_file(serverContext, key.c_str(), SSL_FILETYPE_PEM) != SSL_SUCCESS ||
	    ssl::SSL_CTX_use_PrivateKey_file(serverALPNContext, key.c_str(), SSL_FILETYPE_PEM) != SSL_SUCCESS ||
	    ssl::SSL_CTX_use_PrivateKey_file(clientALPNContext, key.c_str(), SSL_FILETYPE_PEM) != SSL_SUCCESS ||
	    ssl::SSL_CTX_use_PrivateKey_file(clientContext, key.c_str(), SSL_FILETYPE_PEM) != SSL_SUCCESS
	)
	{
...
void CryptoManager::setCertPaths() {
	...
	auto privPath = Util::getLocalPath() + "Certificates" PATH_SEPARATOR_STR "client.key";
	auto certPath = Util::getLocalPath() + "Certificates" PATH_SEPARATOR_STR "client.crt";
