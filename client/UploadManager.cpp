...
bool UploadManager::handleBan(UserConnection* aSource/*, bool forceBan, bool noChecks*/)
{
	...
	//2) "автобан" по шаре максимум ограничить размером "сво€ шара" (но не более 20 гиг)
	//3) "автобан" по слотам максимум ограничить размером "сколько слотов у мен€"/2 (но не более 15 слотов)
	//4) "автобан" по лимиттеру максимум ограничить размером "лимиттер у мен€"/2 (но не выше 60 кб/сек)
	*/
		...
		if (l_User.user)
		{
			...
			if (l_count_dos > l_count_attempts)
			{
				...
				if (l_is_TypePartialTree)
				{
					/*
					Fix
					[2017-10-01 16:20:21] <MikeKMV> оказываетс€ флай умеет флудить
