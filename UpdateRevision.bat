...
set RELEASE_UPDATE_URL="http://update.fly-server.ru/update/6xx/release/"

set BETA_UPDATE_FOLDER=".\update\"
set BETA_UPDATE_URL="http://update.fly-server.ru/update/6xx/beta/"

if "%BETA_STATE%"=="1" (set SYMBOL_SERVER_APP_KEY=6B1B86A3-9825-4906-9481-B2D616F9B213) else (set SYMBOL_SERVER_APP_KEY=5A1ACE32-DFDB-4B2B-AE18-A4078DE8F6F5)
...
:WriteRevision
rem echo SYMUPLOAD %SYMBOL_SERVER_APP_KEY% %APP_VERSION_FOR_SYMBOL_SERVER%.%VERSION_NUM%.%~1 0 FlylinkDC.pdb> SymRegisterBinaries.bat
rem echo SYMUPLOAD %SYMBOL_SERVER_APP_KEY% %APP_VERSION_FOR_SYMBOL_SERVER%.%VERSION_NUM%.%~1 0 FlylinkDC.exe>> SymRegisterBinaries.bat
...
if "%BETA_STATE%"=="1" (echo UpdateMaker.exe -V%~1 -D%BETA_UPDATE_FOLDER% -U%BETA_UPDATE_URL% -B -MP -P"6">> CreateUpdate.bat) else (echo UpdateMaker.exe -V%~1 -D%RELEASE_UPDATE_FOLDER% -U%RELEASE_UPDATE_URL% -R -MP -P"6">> CreateUpdate.bat)
